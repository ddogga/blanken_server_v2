...
-- - `created_at` 을 서로 다르게 흩어 둔다 — 최신순(CREATE_AT_DESC) 정렬을 눈으로 확인하려면 필요하다.
-- - `like_count` 도 흩어 둔다 (인기순 확인용). 실제 서비스에서는 Redis 배치 flush 로만 갱신되는 값이다.
-- - PRIVATE 한 건을 섞어 둔다. 검색이 공개 퀴즈셋만 노출하는지 확인하는 용도다.
-- ──────────────────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────────────────
-- 퀴즈 — 퀴즈셋마다 `quiz_count` 개 (5~15), 총 191개
--
